...
-- VERSION CORRIGÉE : fonctions dans schéma public (pas auth)
...
-- ════════════════════════════════════════════════════════════
-- FONCTIONS (schéma public uniquement — pas de schéma auth)
-- ════════════════════════════════════════════════════════════

-- Retourne l'org_id de l'utilisateur courant depuis le JWT
...
-- Retourne l'org_id (alias utilisé dans les policies RLS)
CREATE OR REPLACE FUNCTION public.solvid_org_id()
RETURNS TEXT
LANGUAGE sql
STABLE SECURITY DEFINER
AS $$
...
CREATE OR REPLACE FUNCTION public.is_solvid_admin()
RETURNS boolean
LANGUAGE sql
STABLE SECURITY DEFINER
AS $$
...
-- ════════════════════════════════════════════════════════════
-- ROW LEVEL SECURITY
-- (utilise public.is_solvid_admin() et public.solvid_org_id())
-- ════════════════════════════════════════════════════════════
...
CREATE POLICY "dossiers_select" ON dossiers FOR SELECT USING (public.is_solvid_admin() OR organization_id = public.solvid_org_id());
CREATE POLICY "dossiers_insert" ON dossiers FOR INSERT WITH CHECK (public.is_solvid_admin() OR organization_id = public.solvid_org_id());
CREATE POLICY "dossiers_update" ON dossiers FOR UPDATE USING (public.is_solvid_admin() OR organization_id = public.solvid_org_id());
CREATE POLICY "dossiers_delete" ON dossiers FOR DELETE USING (public.is_solvid_admin() OR organization_id = public.solvid_org_id());
...
CREATE POLICY "vsme_values_select" ON vsme_values FOR SELECT USING (public.is_solvid_admin() OR organization_id = public.solvid_org_id());
CREATE POLICY "vsme_values_insert" ON vsme_values FOR INSERT WITH CHECK (public.is_solvid_admin() OR organization_id = public.solvid_org_id());
CREATE POLICY "vsme_values_update" ON vsme_values FOR UPDATE USING (public.is_solvid_admin() OR organization_id = public.solvid_org_id());
CREATE POLICY "vsme_values_delete" ON vsme_values FOR DELETE USING (public.is_solvid_admin() OR organization_id = public.solvid_org_id());
...
CREATE POLICY "mission_notes_select" ON mission_notes FOR SELECT USING (public.is_solvid_admin() OR organization_id = public.solvid_org_id());
CREATE POLICY "mission_notes_insert" ON mission_notes FOR INSERT WITH CHECK (public.is_solvid_admin() OR organization_id = public.solvid_org_id());
CREATE POLICY "mission_notes_update" ON mission_notes FOR UPDATE USING (public.is_solvid_admin() OR organization_id = public.solvid_org_id());
CREATE POLICY "mission_notes_delete" ON mission_notes FOR DELETE USING (public.is_solvid_admin() OR organization_id = public.solvid_org_id());
...
CREATE POLICY "pack_instances_select" ON pack_instances FOR SELECT USING (public.is_solvid_admin() OR organization_id = public.solvid_org_id());
CREATE POLICY "pack_instances_insert" ON pack_instances FOR INSERT WITH CHECK (public.is_solvid_admin() OR organization_id = public.solvid_org_id());
CREATE POLICY "pack_instances_update" ON pack_instances FOR UPDATE USING (public.is_solvid_admin() OR organization_id = public.solvid_org_id());
CREATE POLICY "pack_instances_delete" ON pack_instances FOR DELETE USING (public.is_solvid_admin() OR organization_id = public.solvid_org_id());
