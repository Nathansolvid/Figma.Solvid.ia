...
function AuditTrailIcon({ code, dossierId }: { code: string; dossierId: string }) {
  ...
  const loadAudit = () => {
    try {
      ...
      const all: AuditEntry[] = JSON.parse(localStorage.getItem(auditKey) || '[]');
      const filtered = all.filter(e => e.code === code).reverse().slice(0, 20);
      setEntries(filtered);
    } catch {
      setEntries([]);
    }
  ...
  return (
    <div className="relative">
      ...
      {open && (
        ...
        >
          <div
            className="bg-white rounded-xl shadow-2xl max-w-lg w-full mx-4 max-h-[70vh] overflow-hidden"
            onClick={e => e.stopPropagation()}
          >
            <div className="flex items-center justify-between px-5 py-3 border-b" style={{ background: '#f8fdf9' }}>
              <div>
                <h3 className="font-semibold text-sm" style={{ color: '#0A3B2E' }}>
                  Audit Trail — {code}
                </h3>
                ...
                <table className="w-full text-sm">
                  <thead className="sticky top-0 bg-gray-50">
                    <tr className="border-b">
                      <th className="px-4 py-2 text-left text-xs font-semibold" style={{ color: '#6b7280' }}>Date</th>
                      <th className="px-4 py-2 text-left text-xs font-semibold" style={{ color: '#6b7280' }}>Utilisateur</th>
                      <th className="px-4 py-2 text-left text-xs font-semibold" style={{ color: '#6b7280' }}>Ancienne valeur</th>
                      <th className="px-4 py-2 text-left text-xs font-semibold" style={{ color: '#6b7280' }}>Nouvelle valeur</th>
                    </tr>
                  </thead>
                  <tbody>
                    {entries.map((entry, i) => (
                      <tr key={i} className="border-b last:border-0 hover:bg-gray-50">
                        <td className="px-4 py-2 text-xs whitespace-nowrap" style={{ color: '#4b5563' }}>
                          {new Date(entry.timestamp).toLocaleString('fr-FR', {
                            day: '2-digit', month: '2-digit', year: '2-digit',
                            hour: '2-digit', minute: '2-digit',
                          })}
                        </td>
                        <td className="px-4 py-2 text-xs" style={{ color: '#1a2e24' }}>
                          {entry.userId}
                        </td>
                        <td className="px-4 py-2 text-xs font-mono" style={{ color: '#ef4444' }}>
                          {entry.oldValue || '—'}
                        </td>
                        <td className="px-4 py-2 text-xs font-mono" style={{ color: '#059669' }}>
                          {entry.newValue || '—'}
                        </td>
                      </tr>
                    ))}
  ...
  return (
    ...
    >
      ...
      <td className="w-8 py-3 text-center">
        <AuditTrailIcon code={dp.code} dossierId={dossierId} />
      </td>
                          ...
                          <table className="w-full text-left">
                            <thead>
                              <tr style={{ borderBottom: "1px solid #f1f5f1" }}>
                                ...
                                <th className="w-8 py-2 text-xs font-semibold text-center" style={{ color: "#9ca3af" }} title="Audit trail">
                                  <History className="w-3 h-3 mx-auto" />
                                </th>
                                {periods.length > 1 && previousPeriod && (
